...
from .frame_pack.hunyuan_video_packed_inference import HunyuanVideoTransformer3DModelPackedInference
from .frame_pack.utils import crop_or_pad_yield_mask, soft_append_bcthw
...
from .hv_generate_video import get_time_flag, save_images_grid, save_videos_grid, synchronize_device
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sample_solver", type=str, default="unipc", choices=["unipc", "dpm++", "vanilla"],
        help="The solver used to sample."
    )

    parser.add_argument("--dit", type=str, default=None, help="DiT directory or path")
    parser.add_argument(
        "--disable_numpy_memmap", action="store_true",
        help="Disable numpy memmap when loading safetensors. Default is False."
    )
    ...
    parser.add_argument(
        "--control_image_path", type=str, default=None, nargs="*",
    ...
    parser.add_argument("--latent_window_size", type=int, default=9,
                        help="latent window size, default is 9. should not change.")
    parser.add_argument(
        "--embedded_cfg_scale", type=float, default=10.0,
        help="Embeded CFG scale (distilled CFG Scale), default is 10.0"
    )
    parser.add_argument(
        "--guidance_scale",
        type=float,
    ...
    parser.add_argument("--guidance_rescale", type=float, default=0.0,
                        help="CFG Re-scale, default is 0.0. Should not change.")
    ...
    parser.add_argument(
        "--rope_scaling_factor", type=float, default=0.5,
        help="RoPE scaling factor for high resolution (H/W), default is 0.5"
    )
    parser.add_argument(
    ...
    parser.add_argument("--magcache_retention_ratio", type=float, default=0.2,
                        help="MagCache retention ratio, default is 0.2")
    ...
    parser.add_argument("--use_rev", action="store_true", help="use rev")
    ...
    if args.lycoris:
        ...
        if args.fp8_scaled:
            ...
            state_dict = model.fp8_optimization(state_dict, device, move_to_device,
                                                use_scaled_mm=False)  # args.fp8_fast)
    ...
    if not args.fp8_scaled:
        ...
        if target_device is not None and target_dtype is not None:
            model.to(target_device,
                     target_dtype)  # move and cast  at the same time. this reduces redundant copy operations
...
def optimize_model(model: HunyuanVideoTransformer3DModelPackedInference, args: argparse.Namespace,
...
def decode_latent(
    ...
    if not bulk_decode and not one_frame_inference_mode:
        ...
        if not one_frame_inference_mode:
            ...
            # one frame inference
            history_pixels = [hunyuan.vae_decode(latent[:, :, i: i + 1, :, :], vae).cpu() for i in
                              range(latent.shape[2])]
        ...
        for index, prompt in section_prompts.items():
            if prompt in conds_cache:
                ...
                llama_vec, clip_l_pooler = hunyuan.encode_prompt_conds(
                    prompt, text_encoder1, text_encoder2, tokenizer1, tokenizer2,
...
def prepare_i2v_inputs(
    ...
    image_data = prepare_image_inputs(args, device, vae, feature_extractor, image_encoder, shared_models)
    text_data = prepare_text_inputs(args, device, tokenizer1, text_encoder1, tokenizer2, text_encoder2, shared_models)
...
def convert_lora_from_diffusion_pipe_or_something(lora_sd: dict[str, torch.Tensor], prefix: str) -> dict[
    str, torch.Tensor]:
    """
    ...
    for key, weight in lora_sd.items():
        ...
        new_key = f"{prefix}{key_body}".replace(".", "_").replace("_lora_A_", ".lora_down.").replace("_lora_B_",
                                                                                                     ".lora_up.")
...
def encode_text_with_learnable_tokens(
        sa_file,
    ...
    token_file = sa_file[0].replace(".safetensors", "_text_tokens.pt")
    text_tokens = torch.load(token_file)
    selected = text_tokens["text_token_rev"].to(dtype) if use_rev else text_tokens["text_token"].to(dtype)
    selected = selected.to(dtype=encoder_hidden_states.dtype, device=encoder_hidden_states.device, copy=False)
    selected = selected.repeat(B, 1, 1)  # [1, 1, 4096] → [B, 1, D]
    combined_states = encoder_hidden_states[:, :-1, :]
    encoder_hidden_states = torch.cat([selected, combined_states], dim=1)

    B, L = encoder_attention_mask.shape
    combined_mask = encoder_attention_mask[:, :-1]
    encoder_attention_mask = torch.cat([
        torch.ones(B, 1, device=encoder_attention_mask.device,
                   dtype=encoder_attention_mask.dtype),  # 可学习 token 可见
        combined_mask], dim=1)
...
def generate(
        ...
        feature_extractor, image_encoder, tokenizer1, text_encoder1,
        tokenizer2, text_encoder2, model, use_rev,
        shared_models: Optional[Dict] = None,
    ...
    height, width, video_seconds, context, context_null, context_img, end_latent, control_latents, control_mask_images = (
        prepare_i2v_inputs(args, device, vae_instance_for_return, feature_extractor,
                           image_encoder,
                           tokenizer1, text_encoder1,
                           tokenizer2, text_encoder2, shared_models)  # Pass VAE
    )
    ...
    if one_frame_inference is not None:
        ...
        desired_l = total_latent_sections * latent_window_size
        start_latent = context_img[0]["start_latent"]
        t_values = torch.linspace(0, 1, desired_l, device=start_latent.device, dtype=start_latent.dtype)
        interpolated = torch.lerp(
            start_latent.expand(-1, -1, desired_l, -1, -1),
            end_latent.expand(-1, -1, desired_l, -1, -1),
            t_values.view(1, 1, desired_l, 1, 1)
        )
        ...
        for loop_index in range(total_latent_sections):
            ...
            if not f1_mode:
                ...
                clean_latents_4x, clean_latents_2x, clean_latents_1x = history_latents[:, :, -sum([16, 2, 1]):, :,
                                                                       :].split(
                    [16, 2, 1], dim=2
                )
            ...
            llama_vec_token, llama_attention_mask_token = encode_text_with_learnable_tokens(args.lora_weight, llama_vec,
                                                                                            llama_attention_mask,
                                                                                            use_rev)
            # latent_indices = torch.flip(latent_indices, dims=[1])
            ...
            generated_latents = sample_hunyuan(
                ...
                generator=seed_g,
                prompt_embeds=llama_vec_token,
                prompt_embeds_mask=llama_attention_mask_token,
...
def save_video(
        video: torch.Tensor, args: argparse.Namespace, original_base_name: Optional[str] = None,
    ...
    video_path = f"{save_path}/{original_base_name}.mp4"
...
def save_output(
    ...
    total_latent_sections = int(max(round(total_latent_sections), 1))
    video = decode_latent(
        args.latent_window_size, total_latent_sections, args.bulk_decode, vae, latent, device,
    ...
    if args.output_type == "video" or args.output_type == "both":
        ...
        original_name = input_image_name
    ...
    with torch.no_grad():
        for i, prompt_args_item in enumerate(all_prompt_args_list):
            ...
            try:
                ...
                logger.error(f"Error generating latent for prompt: {prompt_args_item.prompt}. Error: {e}",
                             exc_info=True)
...
def main():
    ...
    returned_vae, latent = generate(args, gen_settings, feature_extractor, image_encoder, tokenizer1,
                                    text_encoder1,
                                    tokenizer2, text_encoder2, model, args.use_rev)

    # Save latent and video
    # returned_vae from generate will be used for decoding here.
    save_output(args, returned_vae, latent[0], device, "output")
