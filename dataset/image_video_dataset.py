...
def save_latent_cache_framepack_image(
        item_info: ItemInfo,
        latent: torch.Tensor,
        latent_indices: torch.Tensor,
        clean_latents: torch.Tensor,
    ...
    if clean_latents_4x is not None:
        sd[f"latents_clean_4x_{F}x{H}x{W}_{dtype_str}"] = clean_latents_4x.detach().cpu().contiguous()
    sd[f"content_{F}x{H}x{W}_{dtype_str}"] = content.detach().cpu().contiguous()
...
class BucketBatchManager:
    ...
    def __getitem__(self, idx):
        ...
        batch_tensor_data_rev = {}
        varlen_keys = set()
        for item_info in bucket[start:end]:
            latent_cache_path = item_info.latent_cache_path.replace("cache_directory_stride_2_zz",
                                                                    "cache_directory_stride_2")
            text_encoder_output_cache_path = item_info.text_encoder_output_cache_path.replace(
                "cache_directory_stride_2_zz", "cache_directory_stride_2")
            sd_latent = load_file(item_info.latent_cache_path)
            sd_te = load_file(item_info.text_encoder_output_cache_path)
            sd = {**sd_latent, **sd_te}
            ...
            for key in sd.keys():
                ...
                if content_key not in batch_tensor_data_rev:
                    batch_tensor_data_rev[content_key] = []
                batch_tensor_data_rev[content_key].append(sd[key])

                if is_varlen_key:
                    varlen_keys.add(content_key)

        for key in batch_tensor_data_rev.keys():
            if key not in varlen_keys:
                batch_tensor_data_rev[key] = torch.stack(batch_tensor_data_rev[key])

        if self.timestep_pool is not None:
            batch_tensor_data_rev["timesteps"] = self.timestep_pool[idx][
                                                 : end - start]  # use the pre-generated timesteps
        else:
            batch_tensor_data_rev["timesteps"] = None

        return batch_tensor_data, batch_tensor_data_rev
        ...
        def aggregate_future(consume_all: bool = False):
            while len(futures) >= num_workers or (consume_all and len(futures) > 0):
                ...
                for future in completed_futures:
                    ...
                    #     selected_indices = indices[:target_length]
                    # else:
                    ...
                    target_length = 73
                    if frame_count < 2:
                        selected_indices = [0] * target_length if frame_count == 1 else [0] * target_length
                    else:
                        if frame_count <= target_length:
                            ...
                            middle_count = target_length - 2  # 71
                            middle_indices = np.linspace(1, frame_count - 2, middle_count, dtype=int)
                            selected_indices = [0] + middle_indices.tolist() + [frame_count - 1]
