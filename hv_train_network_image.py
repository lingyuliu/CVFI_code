    ...
    kwargs_handlers = [
        (
            InitProcessGroupKwargs(
                ...
                init_method=(
                    "env://?use_libuv=False" if os.name == "nt" and Version(torch.__version__) >= Version(
                        "2.4.0") else None
                ),
...
class NetworkTrainer:
    ...
    def generate_step_logs(
        ...
        if current_loss_f:
            logs["loss/current_f"] = current_loss_f
            logs["loss/average_f"] = avr_loss_f

        if current_loss_r:
            logs["loss/current_r"] = current_loss_r
            logs["loss/average_r"] = avr_loss_r

        if current_loss_c:
            logs["loss/current_c"] = current_loss_c
            logs["loss/average_c"] = avr_loss_c

        if current_loss_c_rev:
            logs["loss/current_c_rev"] = current_loss_c_rev
            logs["loss/average_c_rev"] = avr_loss_c_rev
        ...
        if optimizer_type.endswith("8bit".lower()):
            ...
            if optimizer_type == "AdamW8bit".lower():
                ...
                trainable_params.append(
                    {'params': [self.text_token, self.text_token_rev], 'lr': 2e-3, 'optim_bits': 32})
            ...
            trainable_params.append({'params': [self.text_token, self.text_token_rev], 'lr': 2e-3})
            optimizer = optimizer_class(trainable_params, lr=lr, **optimizer_kwargs)
    ...
    def get_lr_scheduler(self, args, optimizer: torch.optim.Optimizer, num_processes: int):
        ...
        num_decay_steps: Optional[int] = (
            int(args.lr_decay_steps * num_training_steps) if isinstance(args.lr_decay_steps,
                                                                        float) else args.lr_decay_steps
        )
        ...
        if name == SchedulerType.INVERSE_SQRT:
            return schedule_func(optimizer, num_warmup_steps=num_warmup_steps, timescale=timescale,
                                 **lr_scheduler_kwargs)
    ...
    def get_noisy_model_input_and_timesteps(
        ...
        if (
            ...
            def compute_sampling_timesteps(org_timesteps: Optional[torch.Tensor]) -> torch.Tensor:
                ...
                def rand_logsnr(bs: int, mean: float, std: float,
                                org_ts: Optional[torch.Tensor] = None) -> torch.Tensor:
                    nonlocal device
                    logsnr = (
                ...
                if args.timestep_sampling == "uniform" or args.timestep_sampling == "sigmoid":
                    ...
                    if mid_mask.any():
                        ...
                        if args.timestep_sampling == "qinglong_flux":
                            ...
                            mu = train_utils.get_lin_function(x1=256, y1=0.5, x2=8192, y2=0.9)((h // 2) * (w // 2))
                        shift = math.exp(mu)
                        logits_norm_mid = randn(mid_count,
                                                org_timesteps[mid_mask] if org_timesteps is not None else None)
    ...
    def show_timesteps(self, args: argparse.Namespace):
        ...
        for i in tqdm(range(len(sampled_weighting))):
            ...
            weighting = compute_loss_weighting_for_sd3(args.weighting_scheme, noise_scheduler, timesteps, "cpu",
                                                       torch.float16)
    ...
    def sample_image_inference(self, accelerator, args, transformer, dit_dtype, vae, save_dir, sample_parameter, epoch,
                               steps):
        """architecture independent sample images"""
    ...
    def process_sample_prompts(
        ...
        text_encoder_dtype = torch.float16 if args.text_encoder_dtype is None else model_utils.str_to_dtype(
            args.text_encoder_dtype)
        logger.info(f"loading text encoder 1: {text_encoder1}")
        text_encoder_1 = text_encoder_module.load_text_encoder_1(text_encoder1, accelerator.device, fp8_llm,
                                                                 text_encoder_dtype)
        ...
        with torch.no_grad():
            for i, t in enumerate(tqdm(timesteps, desc=f"Sampling timesteps for prompt {prompt_idx + 1}")):
                ...
                if image_latents is not None:
                    latents_image_input = (
                        image_latents if not do_classifier_free_guidance else torch.cat([image_latents, image_latents],
                                                                                        dim=0)
                    )
    ...
    def load_transformer(
        ...
        transformer = load_transformer(dit_path, attn_mode, split_attn, loading_device, dit_weight_dtype,
    ...
    def unscale_latents(self, latents):
        return latents / vae_module.SCALING_FACTOR
    ...
    def train(self, args):
        ...
        # Load VAE
        vae = self.load_vae(args, vae_dtype=vae_dtype, vae_path=args.vae, device=accelerator.device)
        vae.requires_grad_(False)
        vae.eval()
        ...
        if blocks_to_swap > 0:
            ...
            accelerator.unwrap_model(transformer).move_to_device_except_swap_blocks(
                accelerator.device)  # reduce peak memory usage
        ...
        loss_recorder_f = train_utils.LossRecorder()
        loss_recorder_r = train_utils.LossRecorder()
        loss_recorder_c = train_utils.LossRecorder()
        loss_recorder_c_rev = train_utils.LossRecorder()
        ...
        def save_tokens(ckpt_name: str):
            ckpt_name = ckpt_name.replace(".safetensors", "_text_tokens.pt")
            ckpt_file = os.path.join(args.output_dir, ckpt_name)

            learnable_tokens_state = {
                "text_token": self.text_token.data.detach().cpu().clone(),  # [N, D]
                "text_token_rev": self.text_token_rev.data.detach().cpu().clone(),  # [N, D]
            }

            torch.save(learnable_tokens_state, ckpt_file)
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, (batch, batch_rev) in enumerate(train_dataloader):
                content_keys = [k for k in batch.keys() if k.startswith('content')][0]
                contents = batch[content_keys]
                contents_rev = torch.flip(contents, dims=[2])  #batch_rev[content_keys]
                # print("contents.device:",contents.device)

                with accelerator.accumulate(training_model):
                    accelerator.unwrap_model(network).on_step_start()

                    latents_en = hunyuan.vae_encode(contents, vae)
                    latents_en_rev = hunyuan.vae_encode(contents_rev, vae)
                    # print("vae.device:", vae.device)

                    latents_en = self.scale_shift_latents(latents_en)
                    latents_en_rev = self.scale_shift_latents(latents_en_rev)

                    first_latent = latents_en[:, :, 0, :, :]
                    first_latent_rev = latents_en_rev[:, :, 0, :, :]

                    latents = latents_en[:, :, 1:, :, :]
                    latents_rev = latents_en_rev[:, :, 1:, :, :]

                    # print("latents.shape:", latents.shape)

                    batch["latents"] = latents
                    batch_rev["latents"] = latents_rev

                    #latents = self.scale_shift_latents(latents)
                    #latents_rev = self.scale_shift_latents(latents_rev)
                    ...
                    encoder_hidden_states, encoder_attention_mask = self.encode_text_with_learnable_tokens(
                        batch["llama_vec"], batch["llama_attention_mask"], False)

                    model_pred, target = self.call_dit(
                        args, accelerator, transformer, latents, batch, noise, noisy_model_input, timesteps,
                        network_dtype,
                        encoder_hidden_states, encoder_attention_mask
                    )
                    f_loss = torch.nn.functional.mse_loss(model_pred.to(network_dtype), target, reduction="none")

                    mask_frames = get_mask_frames(
                        step=global_step,
                        total_steps=2000,
                        max_frames=latents.shape[2],
                        min_frames=1
                    )

                    frame_mask = torch.zeros(latents.shape[2], dtype=torch.bool, device=latents.device)
                    selected = torch.randperm(latents.shape[2])[:mask_frames]
                    frame_mask[selected] = True

                    # frame_indices = torch.arange(latents.shape[2], device=latents.device)
                    # frame_mask = frame_indices < visible_frames  # [L], e.g., [True, True, False, ...]

                    latent_mask = frame_mask.unsqueeze(0).unsqueeze(1).unsqueeze(-1).unsqueeze(-1)  # [1, 1, L, 1, 1]

                    # clean_memory_on_device(accelerator.device)
                    # reversed_latents = torch.flip(latents, dims=[2])
                    # reversed_noise = torch.flip(noise, dims=[2])
                    reversed_noise = torch.randn_like(latents_rev)
                    # reversed_noisy_model_input = torch.flip(noisy_model_input, dims=[2])
                    reversed_noisy_model_input, timesteps_rev = self.get_noisy_model_input_and_timesteps(
                        args, reversed_noise, latents_rev, timesteps, noise_scheduler, accelerator.device,
                        dit_dtype
                    )

                    reversed_frame_mask = latent_mask
                    reversed_noisy_model_input = torch.where(reversed_frame_mask, reversed_noisy_model_input, latents_rev)

                    encoder_hidden_states_rev, encoder_attention_mask_rev = self.encode_text_with_learnable_tokens(
                        batch_rev["llama_vec"], batch_rev["llama_attention_mask"], True)

                    # batch_rev["latent_indices"] = torch.flip(batch_rev["latent_indices"], dims=[1])

                    reversed_model_pred, reversed_target = self.call_dit(
                        args, accelerator, transformer, latents_rev, batch_rev, reversed_noise,
                        reversed_noisy_model_input, timesteps_rev,
                        network_dtype, encoder_hidden_states_rev, encoder_attention_mask_rev
                    )
                    # reversed_target = torch.flip(target, dims=[2])
                    reversed_loss = torch.nn.functional.mse_loss(reversed_model_pred.to(network_dtype), reversed_target,
                                                                 reduction="none")
                    reversed_frame_mask = reversed_frame_mask.expand_as(reversed_loss)
                    reversed_loss = reversed_loss * reversed_frame_mask.float()
                    reversed_loss = reversed_loss.sum() / (reversed_frame_mask.sum() + 1e-8)
                    #reversed_loss = reversed_loss[reversed_frame_mask.expand_as(reversed_loss)]
                    # test_loss = encoder_hidden_states[:,-1,:].sum(dim=1)

                    # c_loss = torch.nn.functional.mse_loss(torch.flip(reversed_loss, dims=[2]), model_pred, reduction="none")
                    # c_loss = reversed_loss
                    t_float = timesteps.float() / 1000.0
                    t_float = t_float.view(-1, 1, 1, 1, 1)
                    pred_latents = noisy_model_input - t_float * model_pred
                    model_pred_image = torch.cat([first_latent.unsqueeze(2), pred_latents], dim=2)
                    model_pred_image = self.unscale_latents(model_pred_image)



                    pred_latents_rev = reversed_noisy_model_input - t_float * reversed_model_pred
                    model_pred_image_rev = torch.cat([first_latent_rev.unsqueeze(2), pred_latents_rev], dim=2)
                    model_pred_image_rev = self.unscale_latents(model_pred_image_rev)


                    content_pred = hunyuan.vae_decode(model_pred_image, vae)
                    content_pred_rev = hunyuan.vae_decode(model_pred_image_rev, vae)

                    content_pred = content_pred[:, :, 1:, :, :]  # [B, 3, 36, H, W]
                    content_pred_rev = content_pred_rev[:, :, 1:, :, :]


                    video_time_mask  = frame_mask.repeat_interleave(4)
                    rev_video_frame_mask = video_time_mask.unsqueeze(0).unsqueeze(1).unsqueeze(-1).unsqueeze(-1)



                    c_loss = torch.nn.functional.mse_loss(content_pred, contents[:, :, 1:, :, :], reduction="none")
                    c_loss_rev = torch.nn.functional.mse_loss(content_pred_rev, contents_rev[:, :, 1:, :, :], reduction="none")
                    rev_video_frame_mask = rev_video_frame_mask.expand_as(c_loss_rev)
                    #c_loss_rev = c_loss_rev[rev_video_frame_mask.expand_as(c_loss_rev)]
                    c_loss_rev = c_loss_rev * rev_video_frame_mask.float()
                    c_loss_rev = c_loss_rev.sum() / (rev_video_frame_mask.sum() + 1e-8)


                    if weighting is not None:
                        f_loss = f_loss * weighting
                        reversed_loss = reversed_loss * weighting
                        # c_loss = c_loss * weighting
                    ...
                    loss_latent = f_loss * 1 + reversed_loss * 1
                    image_loss = c_loss * 1 + c_loss_rev * 1

                    loss = loss_latent + image_loss
                    ...
                    if global_step % 100 == 0:
                        print(f"Step {global_step} - text_token[0,0]: {self.text_token[0, 0].item():.6f}")
                        print(
                            f"Grad norm: {self.text_token.grad.norm().item() if self.text_token.grad is not None else 'None!'}")
                        print(self.text_token.requires_grad)
                        print(self.text_token.is_leaf)
                        print("text_token.device:", self.text_token.device)
                        print("text_token in computation:", self.text_token.sum().item())
                        print("encoder_hidden_states.device:", encoder_hidden_states.device)
                        print("encoder_hidden_states.requires_grad:", encoder_hidden_states.requires_grad)
                        print("encoder_hidden_states.grad_fn:", encoder_hidden_states.grad_fn)
                        print("------------")
                        print("foward:", f_loss)
                        print("reverse:", reversed_loss)
                        print("image:", c_loss)
                        print("image_rev:", c_loss_rev)
                        print("model_pred.requires_grad:", model_pred.requires_grad)
                        print("latents mean/std:", latents.mean().item(), latents.std().item())
                        print("latents_rev mean/std:", latents_rev.mean().item(), latents_rev.std().item())
                ...
                if accelerator.sync_gradients:
                    ...
                    if should_sampling or should_saving:
                        optimizer_eval_fn()
                        if should_sampling:
                            self.sample_images(accelerator, args, None, global_step, vae, transformer,
                ...
                current_loss_f = f_loss.detach().item()
                loss_recorder_f.add(epoch=epoch, step=step, loss=current_loss_f)
                avr_loss_f: float = loss_recorder_f.moving_average

                current_loss_r = reversed_loss.detach().item()
                loss_recorder_r.add(epoch=epoch, step=step, loss=current_loss_r)
                avr_loss_r: float = loss_recorder_r.moving_average

                current_loss_c = c_loss.detach().item()
                loss_recorder_c.add(epoch=epoch, step=step, loss=current_loss_c)
                avr_loss_c: float = loss_recorder_c.moving_average

                current_loss_c_rev = c_loss_rev.detach().item()
                loss_recorder_c_rev.add(epoch=epoch, step=step, loss=current_loss_c_rev)
                avr_loss_c_rev: float = loss_recorder_c_rev.moving_average
                ...
                if len(accelerator.trackers) > 0:
                    logs = self.generate_step_logs(
                        args, current_loss, avr_loss, lr_scheduler, lr_descriptions, current_loss_f, avr_loss_f,
                        current_loss_r, avr_loss_r, current_loss_c, avr_loss_c, current_loss_c_rev, avr_loss_c_rev,
            ...
            self.sample_images(accelerator, args, epoch + 1, global_step, vae, transformer, sample_parameters,
                               dit_dtype)
            optimizer_train_fn()
...
def get_mask_frames(step: int, total_steps: int, max_frames: int, min_frames: int = 1) -> int:
    # 线性增长
    progress = min(step / total_steps, 1.0)
    visible = int(min_frames + progress * (max_frames - min_frames))
    return max(visible, min_frames)
...
def setup_parser_common() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--timestep_sampling",
        choices=["sigma", "uniform", "sigmoid", "shift", "flux_shift", "qwen_shift", "logsnr", "qinglong_flux",
                 "qinglong_qwen"],
        default="sigma",
...
def hv_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--dit_in_channels", type=int, default=16,
    ...
    parser.add_argument("--text_encoder1", type=str,
                        help="Text Encoder 1 directory / テキストエンコーダ1のディレクトリ")
    parser.add_argument("--text_encoder2", type=str,
                        help="Text Encoder 2 directory / テキストエンコーダ2のディレクトリ")
    parser.add_argument("--text_encoder_dtype", type=str, default=None,
    ...
    parser.add_argument(
        "--vae_spatial_tile_sample_min_size", type=int, default=None,
