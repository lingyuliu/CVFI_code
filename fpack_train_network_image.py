...
from musubi_tuner.hv_train_network_image import (
    NetworkTrainer,
    load_prompts,
...
class FramePackNetworkTrainer(NetworkTrainer):
    def __init__(self):
        ...
        num_new_tokens = 1
        embedding_dim = 4096
        self.text_token = torch.nn.Parameter(torch.randn(num_new_tokens, embedding_dim) * 0.02,
                                             requires_grad=True)
        self.text_token_rev = torch.nn.Parameter(torch.randn(num_new_tokens, embedding_dim) * 0.02,
                                                 requires_grad=True)
    ...
    def load_vae(self, args: argparse.Namespace, vae_dtype: torch.dtype, vae_path: str, device="cpu"):
        vae_path = args.vae
        logger.info(f"Loading VAE model from {vae_path}")
        vae = load_framepack_vae(args.vae, args.vae_chunk_size, args.vae_spatial_tile_sample_min_size, args.vae_tiling, device)
        return vae
    ...
    def encode_text_with_learnable_tokens(
        ...
        selected = self.text_token_rev if use_rev else self.text_token
        # selected.data = selected.data.to(device)
    ...
    def call_dit(
        ...
        with accelerator.autocast():
            ...
            model_pred = model(
                ...
                timestep=timesteps,
                encoder_hidden_states=encoder_hidden_states, #batch["llama_vec"],
                encoder_attention_mask=encoder_attention_mask, #batch["llama_attention_mask"],
